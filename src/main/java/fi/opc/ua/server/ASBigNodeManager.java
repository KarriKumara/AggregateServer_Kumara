 ...
 */
public class ASBigNodeManager extends NodeManager {
	...
	public class MyBigIoManager extends IoManager {
		...
		protected void readNonValue(ServiceContext serviceContext,
			...
			dataValue.setServerTimestamp(DateTime.currentTime());
			//System.out.println("readNONValue kutsuttu MyBigNodeManagerin puolelta!!!!!!!!!!, ja vikalla rivillä simulate()");
			//simulate();
		}
		...
		protected void readValue(ServiceContext serviceContext, NodeId nodeId,
				...
				DataValue dataValue) throws StatusException {
			System.out.println("readValue kutsuttu MyBigNodeManagerin puolelta!!!!!!!!!!");
			DataItem dataItem = getDataItem(nodeId);
			if (dataItem == null)
	...
	private static final Logger logger = Logger
			.getLogger(ASBigNodeManager.class);
	 ...
	 */
	public ASBigNodeManager(UaServer server, String namespaceUri, int nofItems) {
		...
		dataItems = new TreeMap<String, ASBigNodeManager.DataItem>();
	...
	private void notifyMonitoredDataItems(DataItem dataItem) {
		System.out.println("notifyMonitoredDataItems kutsuttu!!!!!!!!!!!!!(MyBigNodeManager)");
		...
		if (c != null)
			for (MonitoredDataItem item : c) {
				...
				System.out.println("item.notifyDataChange kutsuttu!!!!!!!!!!!!!(MyBigNodeManager @ notifyMonitoredDataItems)");
			}
	}
	...
	protected void afterCreateMonitoredDataItem(ServiceContext serviceContext,
		...
		System.out.println("afterCreateMonitoredDataItem kutsuttu!!!!!!!!!!!!!(MyBigNodeManager)");
		...
		if (c == null) {
			...
			System.out.println("dataItemName lisätty monitoredItems listaan!!!!!!!!!!!!!(MyBigNodeManager @ afterCreateMonitoredDataItem)");
		}
		c.add(item);
