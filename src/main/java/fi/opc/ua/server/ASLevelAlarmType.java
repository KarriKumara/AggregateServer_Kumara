...
public class ASLevelAlarmType extends ExclusiveLevelAlarmTypeNode {
	 ...
	 *
	 */
	private final ASNodeManager myNodeManager;

	public ASLevelAlarmType(ASNodeManager myNodeManager,
	...
	protected void activateAlarm(int severity, DateTime activeTime) {
		...
		if (isEnabled()
			...
			ASNodeManager.println("activateAlarm: severity=" + severity);
			setActive(true);
			setRetain(true);
	...
	protected void inactivateAlarm(DateTime activeTime) {
		if (isEnabled() && isActive()) {
			ASNodeManager.println("inactivateAlarm");
			setActive(false);
