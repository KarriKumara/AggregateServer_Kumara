...
public class MappingEngine {
	...
	private void executeRule(MatchingRule mRule, TargetServer ts) throws StatusException, ServiceException, AddressSpaceException {
		...
		if(mRule.OriginalRule.Type.toLowerCase().equals("deep copy")) {
	        ...
	        System.out.println();
		} else if(mRule.OriginalRule.Type.toLowerCase().equals("deep copy no history")) {
			System.out.println("*** Mapping children nodes for Deep Copy Without Historizing***");
			
			LHSRuleNode lastRuleNode = mRule.LHSNodes.get(mRule.LHSNodes.size() - 1);
			UaNode sourceNode = ts.getTargetServerAddressSpace().getNode(lastRuleNode.MatchingNodeId);
			
			deepCopyNodesNoHistory(sourceNode, currentNode, ts);
	...
	//recursive method for deep copying from 'sourceNode' to 'parent'
	private UaNode deepCopyNodesNoHistory(UaNode sourceNode, UaNode parentNode, TargetServer ts) throws StatusException, ServiceException, AddressSpaceException {
		UaNode mappedNode = null;
		...
		for(UaNode component : components) {
			mappedNode = copyNode(new RHSRuleNode(""), component, parentNode, ts);
			if(mappedNode.getNodeClass().equals(NodeClass.Variable))
			{
				UaVariableNode mappedVariable = (UaVariableNode)mappedNode;
				mappedVariable.setHistorizing(false);
				deepCopyNodesNoHistory(component, mappedVariable, ts);
			} else
			{
				deepCopyNodesNoHistory(component, mappedNode, ts);
			}
		}
